...
var textoId = document.querySelector("#createdBy")

textoId.innerHTML += " Gaspar Moncivaez"
...
function buildToDo(todo, index) {
  ...
  toDoText.id = index; // toDoText.id = index.toString();

  if(todo.complete){
    toDoText.className = "completeText" //  toDoText.classList.add("completeText")
  }
...
function buildToDos(toDos) {
  ...
  var elemementosToDo = toDos.map((todo, index) => buildToDo(todo, index))

  return elemementosToDo;
}
...
function displayToDos() {
  ...
  var toDoItemsElements = buildToDos(toDoItems);

  toDoItemsElements.forEach(element => { toDoContainer.appendChild(element)});
...
function addToDo() {
  ...
  var toDoadd = toDoInput.value;

  var nuevoToDo = new ToDo(toDoadd);
